...
Note to packagers:
 ...
 - Default keybindings have been moved to external tigrc file, which is
   installed to ${sysconfdir} by default. As a fallback, a version of the tigrc
   file is also included inside the binary. Pass `NO_BUILTIN_TIGRC=y` to not
